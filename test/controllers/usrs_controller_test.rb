...
class UsrsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create usr" do
    assert_difference('Usr.count') do
      post usrs_url, params: { usr: { email: @usr.email, fName: @usr.fName, lName: @usr.lName, pass: @usr.pass } }
    end
  ...
  test "should update usr" do
    patch usr_url(@usr), params: { usr: { email: @usr.email, fName: @usr.fName, lName: @usr.lName, pass: @usr.pass } }
